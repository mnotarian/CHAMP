...
def _run_leiden_parallel_CPM(gfile_gamma_nruns_weight_subset_attribute_niters):
	'''
	Parallel wrapper with single argument input for calling :meth:`leiden_ext.run_leiden`
	Unpacks tuple argument and calls run_leiden with arguments.
	:param gfile_att_2_id_dict_shared_gamma_runs_weight: tuple or list of arguments to supply
...
def parallel_leiden_CPM(graph,start=0,fin=1,numruns=200,maxpt=None,niterations=5,
					 nrepeats=1,uselogspace=False,
					 numprocesses=None, attribute=None,weight=None,
					 node_subset=None,progress=None,calc_sim_mat=True):
	'''
...
def run_leiden_windows_CPM(graph,gamma,nruns,weight=None,node_subset=None,attribute=None,
					   output_dictionary=False,niterations=5,calc_sim_mat=True):
	'''
	Windows version takes the input graph directly (not file).
